...
进程重启过(内存热缓存清空,但按 §10.3 磁盘缓存理应救回来)。09-16 回来发的
第一轮,体感 ~4 分钟,日志实锤:
...
- **任何 prompt 静态头部变化都会付一次"全量税"**:跨天(日期字段)、ZCode
  升级(系统提示/工具 schema 变化)、AGENTS.md 改动。82k 上下文 ≈ 4 分钟,
  20k ≈ 1 分钟,随上下文线性增长。
- 服务端配置救不了(加大磁盘预算、换缓存参数都没用);唯一根治是让客户端把
  易变字段挪到 prompt 最尾部(会话历史之后),不归 mlx-serve 管。
- 日常含义:**每天的第一轮固定慢一次**,这是已知且可预期的,不必排查;
