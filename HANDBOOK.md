...
- **内存热缓存随进程死,磁盘缓存跨重启存活**。进程启动时扫描
  ...
  ... (+1 chunks)`)。
- **前缀匹配是 token 精确前缀**。三个失效场景:① 关闭 ZCode 期间会话历史
  继续增长(重开后第一枪要重算差异点之后);② `--prefix-cache-entries 16`
  的 LRU 把旧条目挤出磁盘预算;③ prompt 头部静态区本身变化——跨天日期变了
  或 ZCode 升级改了系统提示/工具定义,见 §10.4。
  日常"同一天内关掉再打开接着聊"无损。
...
### 10.4 跨天的第一轮必然全量 prefill:系统提示里的日期会断前缀(2026-09-16 实测)

场景:09-14 会话一切正常(每轮 99.7% 命中),用户离开两天;期间 mlx-serve
进程也重启过(但 MLX 重启是无辜的——磁盘缓存按 §10.3 本可救回,真凶在 ZCode 侧:
重开会话时重渲染了日期)。09-16 回来发的第一轮,体感 ~4 分钟,日志实锤:

```
<- 82488+175 tokens streamed [prefill: 323.8 tok/s (8192 cached / 82488 total), decode: 20.9 tok/s]
```

74k token 冷 prefill ÷ 324 tok/s ≈ 230 s——**缓存没坏,是前缀在 8192 处断了**。
下一轮立即恢复 `reused 82457/82711`(99.7%),重算的 74k 也已重新落盘。

根因:ZCode 的 prompt 结构是「系统提示 + AGENTS.md + 环境信息 + currentDate
(~8k token) → 会话历史」。日期行位于静态区尾部,`Today's date is 09-14 →
09-16` 一变,从第 9 个 1024-token 块起全部作废(8192 = 8×1024,恰好断在块边界)。
会话历史这 74k 一个字节都没动,但**每个 token 的 KV 数学上依赖它前面的全部
token**——前缀身份变了,磁盘上存的 KV 就无法复用。这不是 bug,是精确前缀缓存的
结构性代价。

要点与推论:

- **任何 prompt 静态头部变化都会付一次"全量税"**:跨天**重开**会话(日期字段被
  重写)、ZCode 升级(系统提示/工具 schema 变化)、AGENTS.md 改动。
  82k 上下文 ≈ 4 分钟,20k ≈ 1 分钟,随上下文线性增长。
- 服务端配置救不了(加大磁盘预算、换缓存参数都没用);唯一根治是让客户端把
  易变字段挪到 prompt 最尾部(会话历史之后),不归 mlx-serve 管。
- 日常含义:税只在"**跨天重开**会话后的第一轮"收——**挂机跨午夜不交税**:
  ZCode 有 date_change 追加机制兜底,前缀稳定,次轮照常 99%+ 命中。
  §10.1 的"重启续接无损"结论有个隐含前提是**同一天内**。
- 伴生观察:跨天第一枪 decode 只有 20.9 tok/s(平时 ~41),是进程重启后 MTP
  投机解码还在 cost table 探测期,次轮恢复正常,与断前缀无关。

代码层面实锤(ZCode 闭源,GitHub 无官方源码;以下从本机 bundle
`/Applications/ZCode.app/Contents/Resources/glm/zcode.cjs` 反推,函数名为混淆名):

- 断前缀路径:每次打开会话都重新执行 `resolveContextSources` 并重建
  `contextSourceSnapshot`,其中 `currentDate` 渲染成 `# currentDate` 块
  (位于 AGENTS.md 之后、会话历史之前的 ~8k 处),跨天重开时被原地重写。
- 正确路径(已存在但没覆盖重开):会话**运行中**跨天时,检测器比对
  `lastEmittedLocalDate`,变化则 `addAttachment("date_change", "The date has
  changed. Today's date is now ...")` 往最新消息**追加**——教科书式 append-only。
- 给官方 issue 的引用点就这两个:快照日期应随会话持久化冻结(重开沿用旧值),
  重开后的日期变化交给现成的 date_change 追加机制,即可全路径免除此税。
